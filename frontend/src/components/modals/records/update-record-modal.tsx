...
import type { UpdateRecordMutationResponse, UpdateRecordMutationVariables } from "@/lib/definitions";
...
export default function UpdateRecordModal({
  ...
  recordId: number,
  recordValue: number,
  mutation: UseMutationResult<UpdateRecordMutationResponse, Error, UpdateRecordMutationVariables, unknown>
}) {
  ...
  return (
      <Modal isOpen={isOpen} placement='auto' onOpenChange={onOpenChange} backdrop="blur" classNames={{base:"dark text-foreground"}} size="md">
        <ModalContent className="w-full max-w-md">
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 text-2xl font-bold items-center justify-center">Update Record</ModalHeader>
              <Divider />
              <Form onSubmit={(e) => onSubmit(e, onClose)} className="w-full mt-3">
                <ModalBody className="w-full">
                  <Input
                    ...
                    type="number"
                    defaultValue={String(recordValue)}
                    classNames={{
                      base: "w-full",
                      input: "w-full"
                    }}
